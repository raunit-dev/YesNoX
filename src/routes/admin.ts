...
interface SettleMarketRequestBody {
...
router.post('/signin', async (req: Request<{}, {}, SigninRequestBody>, res:Response) => {
    try {
        ...
        const token = jwt.sign({ username, role: "admin" }, process.env.ADMIN_SECRET as string, { expiresIn: "2h" });
...
router.post('/market', verifyAdminToken, async (req: Request<{}, {}, CreateMarketRequestBody>, res: Response) => {
    try {
...
router.post('/result', verifyAdminToken, async (req: Request<{}, {},SettleMarketRequestBody>, res: Response) => {
    try {
