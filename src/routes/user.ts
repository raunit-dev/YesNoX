...
import { verifyUserToken } from '../middleware/user.js';
...
router.post('/signup', async (req: Request<{}, {}, SigninRequestBody>, res:Response) => {
    try {
        ...
        const token = jwt.sign({ username, role: "user" }, process.env.USER_SECRET as string, { expiresIn: "2h" });
...
router.post('/split', verifyUserToken, async (req: Request<{}, {}, SplitRequestBody>, res: Response) => {
    try {
...
router.post('/merge', verifyUserToken, async (req: Request<{}, {}, MergeRequestBody>, res: Response) => {
    try {
...
router.post('/claim', verifyUserToken, async (req: Request<{}, {}, ClaimRequestBody>, res: Response) => {
    try {
        const { market_id } = req.body;
        const username = req.auth!.username;
        ...
        const result = await prisma.$transaction(async (tx) => {
            ...
            const marketUpdate: {
                ...
                outcome_a_balance?: number;
                outcome_b_balance?: number;
            } = {
...
router.post('/onramp', verifyUserToken, async (req: Request<{}, {}, OnrampRequestBody>, res: Response) => {
    try {
